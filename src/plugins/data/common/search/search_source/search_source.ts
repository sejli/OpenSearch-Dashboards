...
export class SearchSource {
  ...
  private async fetchExternalSearch(searchRequest: SearchRequest, options: ISearchOptions) {
    const { search, getConfig, onResponse } = this.dependencies;
    // TODO: MQL need to update thtis so it's dependent on what the datasource selected is
    ...
    console.log('params in search source:', params);
        ...
        if ((response as IDataFrameResponse).type === DATA_FRAME_TYPES.DEFAULT) {
          ...
          const handleDirectQuerySuccess = (pollingResult, configurations) => {
            if (pollingResult && pollingResult.body.meta.status === 'SUCCESS') {
              console.log('SEARCH SOURCE POLLING SUCCESS');
